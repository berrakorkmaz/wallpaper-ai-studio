...
import { approveOutput, completeRenderJob, failRenderJob, requestRenderJobs } from "../lib/core/render.ts";
import { changeProductType, createDemoProject, generateUploadedProjectName, markSlotsStaleForNewMaster, setPatternScale, wallpaperScalePolicy } from "../lib/core/project.ts";
...
import { createFalMockupStream, createMockupBatch, getMockupBatch, getPublicRenderConfig, retryMockupSlots } from "../integrations/image-generation/client.ts";
...
type GenerationRun = { slotIds: string[]; active: boolean };
...
async function sourceDataUrl(fileUrl: string) {
  const response = await fetch(fileUrl); if (!response.ok) throw new Error("Source wallpaper could not be read.");
  const blob = await response.blob(); if (!blob.size || blob.size > 20 * 1024 * 1024) throw new Error("Source wallpaper must be smaller than 20 MB for Fal image editing.");
  if (!/^image\/(png|jpeg|webp)$/i.test(blob.type)) throw new Error("Source wallpaper must be PNG, JPG, or WEBP.");
  return await new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onload = () => typeof reader.result === "string" ? resolve(reader.result) : reject(new Error("Source wallpaper could not be encoded.")); reader.onerror = () => reject(new Error("Source wallpaper could not be encoded.")); reader.readAsDataURL(blob); });
}
...
function syncProductionBatch(project: Project, batch: MockupBatchStatus) {
  ...
  for (const remote of batch.outputs) {
    ...
    if (remote.status === "completed" && remote.outputUrl && slot.status !== "ready") {
      ...
      const isPng = remote.outputUrl.startsWith("data:image/png");
      const output: OutputAsset = { id: remote.id, projectId: next.id, userId: next.userId, masterVersionId: job.masterVersionId, slotId: slot.id, role: slot.role, fileUrl: remote.outputUrl, fileName: `${slot.id}.${isPng ? "png" : "jpg"}`, width: remote.width, height: remote.height, format: isPng ? "png" : "jpg", fileSize: 0, productionReady: true, renderProvider: "real", sceneTemplateId: remote.sceneId, approved: false, rejected: false, createdAt: remote.createdAt };
      next = completeRenderJob(next, job.jobId, output); continue;
    }
    if (remote.status === "failed") { next = failRenderJob(next, slot.id, remote.error?.code ?? "FAL_REQUEST_FAILED"); continue; }
...
function Confirmation({ project, back, create, provider }: { project: Project; back: () => void; create: (count: 1 | 6) => void; provider: RuntimeRenderConfig["provider"] }) {
  const master = project.productionMaster!;
  const qa = evaluateMasterQa(project, master, project.artworkAnalysis?.edgeMetrics);
  const falProduction = provider === "fal";
  return <section className="flow-screen"><Progress screen="confirm" /><FlowHeader eyebrow="SON KONTROL" title="Mockupları oluşturmadan önce kontrol edin." copy="Önce tek görselle sabit ölçeği doğrulayabilir veya aynı kategoride altı bağımsız profesyonel kompozisyonu başlatabilirsiniz." /><div className="demo-banner"><b>{falProduction ? "1 Test / 6 Production · Real Fal" : "Demo Mode — Real Fal.ai generation is not connected"}</b><span>{falProduction ? "Her iki seçenek de aynı FLUX.2 scene + FLUX.2 Pro Edit üretim hattını ve aynı kilitli proje ölçeğini kullanır." : "Gösterilen çıktılar yerel önizlemedir; gerçek üretim sonucu olarak sunulmaz."}</span></div><div className="confirmation-grid"><div className="confirm-art"><img src={master.previewUrl || master.fileUrl} alt="Onaylanacak kaynak duvar kâğıdı" /><span>SOURCE ARTWORK · v{master.version}</span></div><div className="confirm-ledger"><div><span>Product</span><b>{project.productType === "seamless" ? "Seamless Pattern" : "Custom Mural"}</b></div><div><span>Primary room</span><b>{project.primaryTargetRoom}</b></div><div><span>Secondary room</span><b>{project.secondaryTargetRoom || "None"}</b></div><div><span>{project.productType === "seamless" ? "Pattern scale" : "Placement"}</span><b>{project.productType === "seamless" ? `${project.patternScale} · ${project.wallpaperScale.repeatWidthCm} × ${project.wallpaperScale.repeatHeightCm} cm locked repeat` : project.artworkPlacementMode.replaceAll("_", " ")}</b></div><div><span>Mockup set</span><b>1 validation or 6 independent scenes</b></div><div><span>Fal model</span><b>{falProduction ? "FLUX.2 scene + Pro Edit" : "Not connected"}</b></div><div><span>Artwork QA</span><b>{qa.status === "QA_PASSED" ? `Passed · ${qa.score}/100` : `Review recommended · ${qa.score}/100`}</b></div></div></div><div className="qa-mini"><b>QA checks</b>{qa.checks.map((item) => <span key={item.key} data-state={item.status}>{item.status === "pass" ? "✓" : item.status === "warn" ? "!" : "×"} {item.label}</span>)}</div><div className="form-actions"><button className="secondary-button" onClick={back}>Ayarları Düzenle</button><div className="generation-actions"><button className="secondary-button" onClick={() => create(1)}>1 Test Mockup Oluştur</button><button className="primary-button" onClick={() => create(6)}>6 Mockup Oluştur →</button></div></div></section>;
}

function Results({ project, update, render, retry, stages, slotErrors, generationRun, generationError, mode, provider, busy, openPreview, openHelp }: { project: Project; update: (project: Project) => void; render: (ids: string[], sourceProject?: Project) => Promise<void>; retry: (slotId: string) => Promise<void>; stages: Record<string, MockupJobStage>; slotErrors: Record<string, string>; generationRun: GenerationRun | null; generationError: string; mode: "demo" | "production"; provider: "mock" | "fal" | "custom"; busy: boolean; openPreview: (asset: OutputAsset) => void; openHelp: (section: string) => void }) {
  ...
  const statusLabel = (slot: Project["slots"][number]) => { const stage = stages[slot.id]; if (stage === "generating_scene") return "Generating room…"; if (stage === "applying_wallpaper" || stage === "detecting_wall" || stage === "compositing_wallpaper") return "Applying wallpaper…"; if (stage === "quality_check") return "Quality check"; if (stage === "completed" || slot.status === "ready") return "Completed"; if (stage === "failed" || slot.status === "failed") return "Generation failed"; if (stage === "queued" || slot.status === "queued") return "Waiting"; return "Waiting"; };
  ...
  const falProduction = provider === "fal";
  const progressIds = generationRun?.slotIds ?? [];
  const completedCount = progressIds.filter((id) => stages[id] === "completed").length;
  const failedCount = progressIds.filter((id) => stages[id] === "failed").length;
  const totalCount = progressIds.length;
  return <section className="flow-screen"><Progress screen="results" /><FlowHeader eyebrow="SONUÇLAR" title="Altı sahneyi ayrı ayrı yönetin." copy={falProduction ? "Final mockuplar, temiz interior scene ve orijinal wallpaper referansı kullanılarak doğrudan FLUX.2 Pro Edit tarafından oluşturulur." : "Her sonuç aynı kaynak görsele bağlı, fakat farklı bir görsel göreve ve kontrollü sahne planına sahiptir."} /><div className="demo-banner"><b>{falProduction ? "Real Fal · FLUX.2 Pro Edit" : mode === "production" ? "Production render pipeline" : "Demo Mode — Real Fal.ai generation is not connected"}</b><span>{falProduction ? "Altı bağımsız oda kontrollü olarak üretilir; sonradan Canvas, rectangle veya wall-mask compositing uygulanmaz." : mode === "production" ? "İşler güvenli backend mockup API üzerinden yürütülür." : "DEMO OUTPUT etiketli görseller, gelecekteki kategori odaklı AI üretimini temsil eden yerel kompozitlerdir."}</span><button onClick={() => openHelp("fal-ai")}>Bağlantı nasıl çalışır?</button></div>{generationRun && <div className="generation-progress" aria-live="polite"><div><span>{generationRun.active ? "Mockuplar hazırlanıyor…" : failedCount ? "Üretim kısmen tamamlandı" : "Mockuplar hazır"}</span><b>{completedCount} / {totalCount} tamamlandı</b>{failedCount > 0 && <em>{failedCount} failed</em>}</div><div className="generation-track" aria-label={`${completedCount} of ${totalCount} completed`}><i style={{ width: `${totalCount ? completedCount / totalCount * 100 : 0}%` }} /></div></div>}{generationError && <p className="form-error" role="alert">{generationError}</p>}<div className="category-switch"><label><span>VISUAL / ROOM CATEGORY</span><select value={project.primaryTargetRoom} disabled={busy} onChange={(event) => changeCategory(event.target.value)}>{rooms.map((room) => <option key={room}>{room}</option>)}</select></label><p>Kategori değiştiğinde altı sahnenin tamamı yeni görsel dünyaya göre hazırlanır.</p></div><div className="results-toolbar"><span><b>{approved.length}</b> / 6 approved</span><button disabled={!selected.length || exporting} onClick={() => exportAssets(selected, "selected-wallpaper-mockups.zip")}>Selected ZIP</button><button className="primary-button" disabled={!approved.length || exporting} onClick={() => exportAssets(approved.map((asset) => asset.id), "approved-wallpaper-mockups.zip")}>ZIP all approved ↓</button></div><div className="result-grid">{project.slots.map((slot, index) => { const asset = project.outputAssets.find((item) => item.id === slot.outputAssetId); const scene = getSceneBlueprint(project.primaryTargetRoom, sceneRoleForSlot(slot), Math.max(0, slot.version - 1)); const stage = stages[slot.id]; const loading = !asset && ["queued", "generating_scene", "applying_wallpaper"].includes(stage ?? ""); const failed = stage === "failed" || slot.status === "failed"; return <article key={slot.id} className={`${asset?.rejected ? "rejected" : asset?.approved ? "approved" : ""}${loading ? " loading-card" : ""}${failed ? " failed-card" : ""}`}><button className="result-preview" disabled={!asset} onClick={() => asset && openPreview(asset)} style={asset ? { backgroundImage: `url(${asset.fileUrl})` } : undefined}><span>M{index + 1}</span><b>{statusLabel(slot)}</b>{!asset && <i className="result-skeleton" aria-hidden="true" />}{asset && <em>{falProduction ? "FLUX.2 PRO EDIT" : asset.productionReady ? "PRODUCTION OUTPUT" : "DEMO OUTPUT"}</em>}</button><div className="result-copy"><small>{scene.roleLabel}</small><h3>{scene.title}</h3><p>{scene.cameraAngle} · {scene.lighting}</p>{failed && <p className="slot-error" role="alert">{slotErrors[slot.id] || "Generation failed. You can retry only this mockup."}</p>}{asset && <label><input type="checkbox" checked={selected.includes(asset.id)} onChange={() => toggle(asset.id)} /> ZIP için seç</label>}</div><div className="result-actions">{asset && <button onClick={() => update(approveOutput(project, asset.id, true))}>{asset.approved ? "Approved ✓" : "Approve"}</button>}{asset && <button onClick={() => update(approveOutput(project, asset.id, false))}>Reject</button>}<button disabled={(mode === "demo" && !asset) || busy} onClick={() => falProduction ? render([slot.id]) : mode === "production" ? retry(slot.id) : render([slot.id])}>{failed ? "Retry" : "Regenerate"}</button>{asset && <button onClick={async () => { const response = await fetch(asset.fileUrl); downloadBlob(await response.blob(), asset.fileName); }}>Download</button>}</div></article>; })}</div></section>;
}
...
function normalizeProject(raw: Project) {
  ...
  return { ...fallback, ...raw, wallpaperScale: raw.wallpaperScale ?? wallpaperScalePolicy(raw.productType, raw.patternScale), prompt: { ...fallback.prompt, ...raw.prompt }, productionMaster: null, activeMasterVersionId: null, masterVersions: [], outputAssets: [], renderJobs: [], slots: fallback.slots, masterStatus: "AWAITING_UPLOAD" as const };
}

export default function StudioApp() {
  const [project, setProject] = useState<Project | null>(null); const [screen, setScreen] = useState<Screen>("home"); const [uploadMode, setUploadMode] = useState<UploadMode>("direct"); const [helpOpen, setHelpOpen] = useState(false); const [helpSection, setHelpSection] = useState<string | null>(null); const [rendering, setRendering] = useState(false); const [preview, setPreview] = useState<OutputAsset | null>(null); const [renderConfig, setRenderConfig] = useState<RuntimeRenderConfig | null>(null); const [renderConfigError, setRenderConfigError] = useState(""); const [remoteBatch, setRemoteBatch] = useState<MockupBatchStatus | null>(null); const [slotStages, setSlotStages] = useState<Record<string, MockupJobStage>>({}); const [slotErrors, setSlotErrors] = useState<Record<string, string>>({}); const [generationRun, setGenerationRun] = useState<GenerationRun | null>(null); const [generationError, setGenerationError] = useState("");
  ...
  const render = async (ids: string[], sourceProject = project) => { if (!sourceProject?.productionMaster || rendering || !renderConfig) return; setRendering(true); setGenerationError(""); try { const qa = evaluateMasterQa(sourceProject, sourceProject.productionMaster, sourceProject.artworkAnalysis?.edgeMetrics); const approvedAt = new Date().toISOString(); const master = { ...sourceProject.productionMaster, approvedAt }; const prepared: Project = { ...sourceProject, qa, masterStatus: "APPROVED", productionMaster: master, masterVersions: sourceProject.masterVersions.map((item) => item.id === master.id ? master : item), artDirection: { ...sourceProject.artDirection, savedAt: sourceProject.artDirection.savedAt || approvedAt } }; if (renderConfig.provider === "fal") {
      const requestedIds = ids;
      const encodedSource = await sourceDataUrl(master.fileUrl);
      const requested = requestRenderJobs(prepared, prepared.userId, requestedIds, "real");
      const baseRequest = buildMockupBatchRequest(prepared, requestedIds, encodedSource);
      setSlotStages((current) => ({ ...current, ...Object.fromEntries(requestedIds.map((id) => [id, "queued" as const])) }));
      setSlotErrors((current) => { const next = { ...current }; for (const id of requestedIds) delete next[id]; return next; });
      setGenerationRun({ slotIds: requestedIds, active: true });
      update(requested.project); setScreen("results");
      let cursor = 0;
      const worker = async () => { while (cursor < baseRequest.scenes.length) {
        const scene = baseRequest.scenes[cursor++];
        const input = { ...baseRequest, scenes: [scene], idempotencyKey: `${baseRequest.idempotencyKey}:${scene.slotId}:${scene.generationSeed}` };
        try {
          const batch = await createFalMockupStream(input, (event) => { if (event.type !== "stage") return; setSlotStages((current) => ({ ...current, [event.slotId]: event.stage })); if (event.error) setSlotErrors((current) => ({ ...current, [event.slotId]: event.error!.message })); });
          setRemoteBatch(batch);
          setProject((current) => current ? { ...syncProductionBatch(current, batch), updatedAt: new Date().toISOString() } : current);
        } catch (error) {
          const message = error instanceof Error ? error.message : "Generation failed. Try this mockup again.";
          setSlotStages((current) => ({ ...current, [scene.slotId]: "failed" }));
          setSlotErrors((current) => ({ ...current, [scene.slotId]: message }));
          setProject((current) => current ? { ...failRenderJob(current, scene.slotId, "FAL_REQUEST_FAILED"), updatedAt: new Date().toISOString() } : current);
        }
      } };
      await Promise.all(Array.from({ length: Math.min(2, baseRequest.scenes.length) }, () => worker()));
      return;
    } if (renderConfig.provider === "custom") { const batch = await createMockupBatch(buildMockupBatchRequest(prepared, ids)); const requested = requestRenderJobs(prepared, prepared.userId, ids, "real"); update(syncProductionBatch(requested.project, batch)); setRemoteBatch(batch); setScreen("results"); return; } const requested = requestRenderJobs(prepared, prepared.userId, ids, "mock"); let current = requested.project; update(current); setScreen("results"); for (const job of requested.jobs) { const output = await compositeOutput(current, job); current = completeRenderJob(current, job.jobId, output); update(current); } } catch (error) { setGenerationError(error instanceof Error ? error.message : "Mockup generation could not be started."); setScreen("results"); } finally { setGenerationRun((current) => current ? { ...current, active: false } : current); setRendering(false); } };
  ...
  return <div className="simple-app"><header className="simple-topbar"><button className="simple-brand" onClick={() => setScreen("home")}><span>W</span><strong>Wallpaper AI Studio<small>MOCKUP WORKSPACE</small></strong></button><div className="top-project"><small>{project.productionMaster ? "ACTIVE PROJECT" : "NEW WORKSPACE"}</small><b>{project.productionMaster ? project.projectName : "Choose how to begin"}</b></div><button className="new-button" onClick={startFresh}>＋ Yeni çalışma</button></header><div className="simple-shell"><aside className="simple-sidebar"><nav aria-label="Ana menü"><button className={screen === "home" ? "active" : ""} onClick={() => setScreen("home")}><span>⌂</span>Ana Sayfa</button><button className={screen === "projects" ? "active" : ""} onClick={() => setScreen("projects")}><span>□</span>Projelerim</button><button onClick={() => openHelp()}><span>?</span>Kullanım Kılavuzu</button><button className={screen === "account" ? "active" : ""} onClick={() => setScreen("account")}><span>⚙</span>Ayarlar</button></nav><div className="mode-card"><b>Render Provider: {renderConfig.provider === "fal" ? "Fal" : renderConfig.provider === "custom" ? "Custom" : "Mock"}</b><span>{renderConfig.provider === "fal" ? renderConfig.configured ? "Real AI renderer" : "Missing FAL_KEY" : renderConfig.provider === "custom" ? "Production renderer" : "Development renderer"}</span></div></aside><main className="simple-main"><div className="simple-content">
    ...
    {screen === "upload" && <section className="flow-screen"><Progress screen="upload" /><FlowHeader eyebrow={uploadMode === "prompt" ? "PROMPT RESULT" : "READY ARTWORK"} title={uploadMode === "prompt" ? "Ürettiğiniz tasarımı ekleyin." : "Hazır tasarımınızı ekleyin."} copy="Kaynak görsel korunur ve her interior scene için FLUX.2 Pro Edit'e orijinal wallpaper referansı olarak gönderilir." /><SourceUpload project={project} mode={uploadMode} onComplete={completeUpload} onBack={() => setScreen(uploadMode === "prompt" ? "prompt" : "home")} /></section>}
    ...
    {screen === "confirm" && project.productionMaster && <Confirmation project={project} back={() => setScreen("settings")} create={(count) => render(project.slots.slice(0, count).map((slot) => slot.id))} provider={renderConfig.provider} />}
    {screen === "results" && <Results project={project} update={update} render={render} retry={retry} stages={{ ...Object.fromEntries((remoteBatch?.outputs ?? []).map((output) => [output.slotId, output.status])), ...slotStages }} slotErrors={slotErrors} generationRun={generationRun} generationError={generationError} mode={renderConfig.mode} provider={renderConfig.provider} busy={rendering} openPreview={setPreview} openHelp={openHelp} />}
