...
function Results({ project, update, render, retry, stages, generationError, mode, busy, openPreview, openHelp }: { project: Project; update: (project: Project) => void; render: (ids: string[], sourceProject?: Project) => Promise<void>; retry: (slotId: string) => Promise<void>; stages: Record<string, MockupJobStage>; generationError: string; mode: "demo" | "production"; busy: boolean; openPreview: (asset: OutputAsset) => void; openHelp: (section: string) => void }) {
...
export default function StudioApp() {
  const [project, setProject] = useState<Project | null>(null); const [screen, setScreen] = useState<Screen>("home"); const [uploadMode, setUploadMode] = useState<UploadMode>("direct"); const [helpOpen, setHelpOpen] = useState(false); const [helpSection, setHelpSection] = useState<string | null>(null); const [rendering, setRendering] = useState(false); const [preview, setPreview] = useState<OutputAsset | null>(null); const [renderConfig, setRenderConfig] = useState<RuntimeRenderConfig>({ mode: "demo", provider: "mock", productionReady: false, missingConfiguration: [] }); const [remoteBatch, setRemoteBatch] = useState<MockupBatchStatus | null>(null); const [generationError, setGenerationError] = useState("");
  ...
  useEffect(() => { void getPublicRenderConfig().then(setRenderConfig).catch(() => setRenderConfig({ mode: "demo", provider: "mock", productionReady: false, missingConfiguration: [] })); }, []);
  ...
  const render = async (ids: string[], sourceProject = project) => { if (!sourceProject?.productionMaster || rendering) return; setRendering(true); setGenerationError(""); try { const qa = evaluateMasterQa(sourceProject, sourceProject.productionMaster, sourceProject.artworkAnalysis?.edgeMetrics); const approvedAt = new Date().toISOString(); const master = { ...sourceProject.productionMaster, approvedAt }; const prepared: Project = { ...sourceProject, qa, masterStatus: "APPROVED", productionMaster: master, masterVersions: sourceProject.masterVersions.map((item) => item.id === master.id ? master : item), artDirection: { ...sourceProject.artDirection, savedAt: sourceProject.artDirection.savedAt || approvedAt } }; if (renderConfig.mode === "production") { const batch = await createMockupBatch(buildMockupBatchRequest(prepared, ids)); const requested = requestRenderJobs(prepared, prepared.userId, ids, "real"); update(requested.project); setRemoteBatch(batch); setScreen("results"); return; } const requested = requestRenderJobs(prepared, prepared.userId, ids, "mock"); let current = requested.project; update(current); setScreen("results"); for (const job of requested.jobs) { const output = await compositeOutput(current, job); current = completeRenderJob(current, job.jobId, output); update(current); } } catch (error) { setGenerationError(error instanceof Error ? error.message : "Mockup generation could not be started."); setScreen("results"); } finally { setRendering(false); } };
  ...
  return <div className="simple-app"><header className="simple-topbar"><button className="simple-brand" onClick={() => setScreen("home")}><span>W</span><strong>Wallpaper AI Studio<small>MOCKUP WORKSPACE</small></strong></button><div className="top-project"><small>{project.productionMaster ? "ACTIVE PROJECT" : "NEW WORKSPACE"}</small><b>{project.productionMaster ? project.projectName : "Choose how to begin"}</b></div><button className="new-button" onClick={startFresh}>＋ Yeni çalışma</button></header><div className="simple-shell"><aside className="simple-sidebar"><nav aria-label="Ana menü"><button className={screen === "home" ? "active" : ""} onClick={() => setScreen("home")}><span>⌂</span>Ana Sayfa</button><button className={screen === "projects" ? "active" : ""} onClick={() => setScreen("projects")}><span>□</span>Projelerim</button><button onClick={() => openHelp()}><span>?</span>Kullanım Kılavuzu</button><button className={screen === "account" ? "active" : ""} onClick={() => setScreen("account")}><span>⚙</span>Ayarlar</button></nav><div className="mode-card"><b>DEMO MODE</b><span>Fal.ai bağlı değil</span></div></aside><main className="simple-main"><div className="simple-content">
    ...
    {screen === "results" && <Results project={project} update={update} render={render} retry={retry} stages={Object.fromEntries((remoteBatch?.outputs ?? []).map((output) => [output.slotId, output.status]))} generationError={generationError} mode={renderConfig.mode} busy={rendering} openPreview={setPreview} openHelp={openHelp} />}
