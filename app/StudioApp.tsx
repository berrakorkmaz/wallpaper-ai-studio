...
function Confirmation({ project, back, create, singleMockupTest }: { project: Project; back: () => void; create: () => void; singleMockupTest: boolean }) {
  ...
  return <section className="flow-screen"><Progress screen="confirm" /><FlowHeader eyebrow="SON KONTROL" title="Mockupları oluşturmadan önce kontrol edin." copy={singleMockupTest ? "Yeni multi-reference Fal pipeline'ını doğrulamak için yalnızca ilk scene blueprint'i üretilecek." : "Oluştur düğmesi yalnızca bir kez altı bağımsız işi başlatır. Bu kurulum demo modundadır ve gerçek Fal.ai bakiyesi kullanmaz."} /><div className="demo-banner"><b>{singleMockupTest ? "Single Mockup · FLUX.2 Pro Edit Test" : "Demo Mode — Real Fal.ai generation is not connected"}</b><span>{singleMockupTest ? "Fal önce temiz odayı üretir, ardından orijinal wallpaper'ı ikinci referans olarak kullanıp final mockup'ı doğrudan image-edit ile oluşturur." : "Gösterilen çıktılar yerel önizlemedir; gerçek üretim sonucu olarak sunulmaz."}</span></div><div className="confirmation-grid"><div className="confirm-art"><img src={master.previewUrl || master.fileUrl} alt="Onaylanacak kaynak duvar kâğıdı" /><span>SOURCE ARTWORK · v{master.version}</span></div><div className="confirm-ledger"><div><span>Product</span><b>{project.productType === "seamless" ? "Seamless Pattern" : "Custom Mural"}</b></div><div><span>Primary room</span><b>{project.primaryTargetRoom}</b></div><div><span>Secondary room</span><b>{project.secondaryTargetRoom || "None"}</b></div><div><span>{project.productType === "seamless" ? "Pattern scale" : "Placement"}</span><b>{project.productType === "seamless" ? project.patternScale : project.artworkPlacementMode.replaceAll("_", " ")}</b></div><div><span>Mockup set</span><b>{singleMockupTest ? "1 multi-reference test mockup" : "6 independent scenes"}</b></div><div><span>Fal model</span><b>{singleMockupTest ? "FLUX.2 scene + Pro Edit" : "Not connected"}</b></div><div><span>Artwork QA</span><b>{qa.status === "QA_PASSED" ? `Passed · ${qa.score}/100` : `Review recommended · ${qa.score}/100`}</b></div></div></div><div className="qa-mini"><b>QA checks</b>{qa.checks.map((item) => <span key={item.key} data-state={item.status}>{item.status === "pass" ? "✓" : item.status === "warn" ? "!" : "×"} {item.label}</span>)}</div><div className="form-actions"><button className="secondary-button" onClick={back}>Ayarları Düzenle</button><button className="primary-button" onClick={create}>{singleMockupTest ? "1 Test Mockup Oluştur" : "6 Mockup Oluştur →"}</button></div></section>;
}
