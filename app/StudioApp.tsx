...
function Confirmation({ project, back, create, falSmokeTest }: { project: Project; back: () => void; create: () => void; falSmokeTest: boolean }) {
  ...
  return <section className="flow-screen"><Progress screen="confirm" /><FlowHeader eyebrow="SON KONTROL" title="Mockupları oluşturmadan önce kontrol edin." copy={falSmokeTest ? "Altı farklı scene blueprint'i Fal ile üretilir; ardından orijinal wallpaper deterministik olarak duvarlara uygulanır." : "Oluştur düğmesi yalnızca bir kez altı bağımsız işi başlatır. Bu kurulum demo modundadır ve gerçek Fal.ai bakiyesi kullanmaz."} /><div className="demo-banner"><b>{falSmokeTest ? "Real Fal + Source-Preserving Compositing" : "Demo Mode — Real Fal.ai generation is not connected"}</b><span>{falSmokeTest ? "Fal odaları üretir; yüklediğiniz orijinal görsel daha sonra sunucuda yeniden çizilmeden uygulanır." : "Gösterilen çıktılar yerel önizlemedir; gerçek üretim sonucu olarak sunulmaz."}</span></div><div className="confirmation-grid"><div className="confirm-art"><img src={master.previewUrl || master.fileUrl} alt="Onaylanacak kaynak duvar kâğıdı" /><span>SOURCE ARTWORK · v{master.version}</span></div><div className="confirm-ledger"><div><span>Product</span><b>{project.productType === "seamless" ? "Seamless Pattern" : "Custom Mural"}</b></div><div><span>Primary room</span><b>{project.primaryTargetRoom}</b></div><div><span>Secondary room</span><b>{project.secondaryTargetRoom || "None"}</b></div><div><span>{project.productType === "seamless" ? "Pattern scale" : "Placement"}</span><b>{project.productType === "seamless" ? project.patternScale : project.artworkPlacementMode.replaceAll("_", " ")}</b></div><div><span>Mockup set</span><b>6 independent scenes</b></div><div><span>Fal model</span><b>{falSmokeTest ? "Server-configured" : "Not connected"}</b></div><div><span>Artwork QA</span><b>{qa.status === "QA_PASSED" ? `Passed · ${qa.score}/100` : `Review recommended · ${qa.score}/100`}</b></div></div></div><div className="qa-mini"><b>QA checks</b>{qa.checks.map((item) => <span key={item.key} data-state={item.status}>{item.status === "pass" ? "✓" : item.status === "warn" ? "!" : "×"} {item.label}</span>)}</div><div className="form-actions"><button className="secondary-button" onClick={back}>Ayarları Düzenle</button><button className="primary-button" onClick={create}>6 Mockup Oluştur →</button></div></section>;
}
