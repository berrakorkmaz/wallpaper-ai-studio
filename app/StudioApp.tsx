...
function Results({ project, update, render, busy, openPreview, openHelp }: { project: Project; update: (project: Project) => void; render: (ids: string[], sourceProject?: Project) => Promise<void>; busy: boolean; openPreview: (asset: OutputAsset) => void; openHelp: (section: string) => void }) {
  ...
  return <section className="flow-screen"><Progress screen="results" /><FlowHeader eyebrow="SONUÇLAR" title="Altı sahneyi ayrı ayrı yönetin." copy="Her sonuç aynı kaynak görsele bağlı, fakat farklı bir görsel göreve ve kontrollü sahne planına sahiptir." /><div className="demo-banner"><b>Demo Mode — Real Fal.ai generation is not connected</b><span>DEMO OUTPUT etiketli görseller, gelecekteki kategori odaklı AI üretimini temsil eden yerel kompozitlerdir.</span><button onClick={() => openHelp("fal-ai")}>Bağlantı nasıl çalışır?</button></div><div className="category-switch"><label><span>VISUAL / ROOM CATEGORY</span><select value={project.primaryTargetRoom} disabled={busy} onChange={(event) => changeCategory(event.target.value)}>{rooms.map((room) => <option key={room}>{room}</option>)}</select></label><p>Kategori değiştiğinde altı sahnenin tamamı yeni görsel dünyaya göre hazırlanır.</p></div><div className="results-toolbar"><span><b>{approved.length}</b> / 6 approved</span><button disabled={!selected.length || exporting} onClick={() => exportAssets(selected, "selected-wallpaper-mockups.zip")}>Selected ZIP</button><button className="primary-button" disabled={!approved.length || exporting} onClick={() => exportAssets(approved.map((asset) => asset.id), "approved-wallpaper-mockups.zip")}>ZIP all approved ↓</button></div><div className="result-grid">{project.slots.map((slot, index) => { const asset = project.outputAssets.find((item) => item.id === slot.outputAssetId); const scene = getSceneBlueprint(project.primaryTargetRoom, sceneRoleForSlot(slot), Math.max(0, slot.version - 1)); return <article key={slot.id} className={asset?.rejected ? "rejected" : asset?.approved ? "approved" : ""}><button className="result-preview" disabled={!asset} onClick={() => asset && openPreview(asset)} style={asset ? { backgroundImage: `url(${asset.fileUrl})` } : undefined}><span>M{index + 1}</span><b>{statusLabel(slot.status)}</b>{asset && <em>DEMO OUTPUT</em>}</button><div className="result-copy"><small>{scene.roleLabel}</small><h3>{scene.title}</h3><p>{scene.cameraAngle} · {scene.lighting}</p>{asset && <label><input type="checkbox" checked={selected.includes(asset.id)} onChange={() => toggle(asset.id)} /> ZIP için seç</label>}</div><div className="result-actions">{asset && <button onClick={() => update(approveOutput(project, asset.id, true))}>{asset.approved ? "Approved ✓" : "Approve"}</button>}{asset && <button onClick={() => update(approveOutput(project, asset.id, false))}>Reject</button>}<button disabled={!asset || busy} onClick={() => render([slot.id])}>Regenerate</button>{asset && <button onClick={async () => { const response = await fetch(asset.fileUrl); downloadBlob(await response.blob(), asset.fileName); }}>Download</button>}</div></article>; })}</div></section>;
}
...
export default function StudioApp() {
  ...
  const render = async (ids: string[], sourceProject = project) => { if (!sourceProject?.productionMaster || rendering) return; setRendering(true); try { const qa = evaluateMasterQa(sourceProject, sourceProject.productionMaster, sourceProject.artworkAnalysis?.edgeMetrics); const approvedAt = new Date().toISOString(); const master = { ...sourceProject.productionMaster, approvedAt }; const prepared: Project = { ...sourceProject, qa, masterStatus: "APPROVED", productionMaster: master, masterVersions: sourceProject.masterVersions.map((item) => item.id === master.id ? master : item), artDirection: { ...sourceProject.artDirection, savedAt: sourceProject.artDirection.savedAt || approvedAt } }; const requested = requestRenderJobs(prepared, prepared.userId, ids, "mock"); let current = requested.project; update(current); setScreen("results"); for (const job of requested.jobs) { const output = await compositeOutput(current, job); current = completeRenderJob(current, job.jobId, output); update(current); } } finally { setRendering(false); } };
