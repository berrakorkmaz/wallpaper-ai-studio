...
import type { ArtworkSource, DesignAsset, OutputAsset, PatternScale, ProductType, Project, PromptSpec, RenderJob } from "../lib/core/types.ts";
...
async function compositeOutput(project: Project, job: RenderJob): Promise<OutputAsset> {
  ...
  context.fillStyle = "#eee8dc"; context.fillRect(0, 0, width, height);
  ...
  const offset = job.slotRole === "Alternate room" ? 180 : 0;
  drawWall(offset, 0, width - offset, height * .78);
  ...
  context.fillStyle = "#d8cbbb"; context.fillRect(0, height * .78, width, height * .22);
  context.fillStyle = job.slotRole === "Alternate room" ? "#8d726a" : "#788479"; roundedRect(context, job.slotRole === "Wide room view" ? 2050 : 520, 1990, job.slotRole === "Close-up detail" ? 480 : 1080, 510, 80);
  context.fillStyle = "rgba(28,32,31,.86)"; context.font = "42px Arial"; context.fillText(`${job.slotRole.toUpperCase()} · DEMO OUTPUT`, 90, height - 75);
...
function PromptBuilder({ project, update, onUpload, onBack }: { project: Project; update: (project: Project) => void; onUpload: () => void; onBack: () => void }) {
  ...
  return <section className="flow-screen"><Progress screen="prompt" /><FlowHeader eyebrow="PROMPT BUILDER" title="Duvar kâğıdı fikrinizi yönlendirin." copy="Seçimlerinizden İngilizce ve üretime uygun bir Midjourney promptu oluşturun. Prompt yalnızca sanat eserini tarif eder; oda veya mobilya üretmez." />
    {!generated ? <div className="flow-card prompt-form"><div className="choice-row"><button className={project.productType === "seamless" ? "selected" : ""} onClick={() => update(preserveArtworkForType(project, "seamless"))}><b>Seamless Pattern</b><small>Tekrarlanabilir kare desen</small></button><button className={project.productType === "mural" ? "selected" : ""} onClick={() => update(preserveArtworkForType(project, "mural"))}><b>Custom Mural</b><small>Panoramik tek kompozisyon</small></button></div><div className="form-grid-simple"><Select label="Theme" value={project.prompt.theme} values={themes} onChange={(value) => setPrompt("theme", value)} /><Select label="Style" value={project.prompt.style} values={styles} onChange={(value) => setPrompt("style", value)} /><Select label="Color palette" value={project.prompt.palette} values={palettes} onChange={(value) => setPrompt("palette", value)} /><Select label="Mood" value={project.mood} values={moods} onChange={(mood) => update({ ...project, mood })} /><Field label="Main motifs" value={project.prompt.motifs} onChange={(value) => setPrompt("motifs", value)} placeholder="foxes, fern leaves, tiny mushrooms" /><Select label="Density" value={project.prompt.density} values={["airy", "balanced", "dense"]} onChange={(value) => setPrompt("density", value as PromptSpec["density"])} /><Field label="Exclude" value={project.prompt.exclusions} onChange={(value) => setPrompt("exclusions", value)} /></div><div className="form-actions"><button className="secondary-button" onClick={onBack}>Geri</button><button className="primary-button" onClick={() => generate()}>Prompt Oluştur ✦</button></div></div> : <div className="prompt-result"><div className="prompt-paper"><span>ENGLISH MIDJOURNEY PROMPT</span><textarea aria-label="Generated Midjourney prompt" value={project.prompt.promptText} onChange={(event) => setPrompt("promptText", event.target.value)} /><div className="prompt-actions"><button onClick={async () => { await navigator.clipboard.writeText(project.prompt.promptText); setCopied(true); }}>{copied ? "Kopyalandı ✓" : "Copy"}</button><button onClick={() => generate(true)}>Regenerate</button><button onClick={() => setGenerated(false)}>Edit Choices</button></div></div><div className="instruction-card"><b>Sonraki adım</b><p>Promptu kopyalayın, Midjourney'de görselinizi üretin ve seçtiğiniz sonucu bilgisayarınıza indirin. Ardından burada yükleyerek mockup akışına devam edin.</p><button className="primary-button" onClick={onUpload}>Midjourney Sonucunu Yükle →</button></div></div>}
  </section>;
}
...
function Confirmation({ project, back, create }: { project: Project; back: () => void; create: () => void }) {
  const master = project.productionMaster!;
  const qa = evaluateMasterQa(project, master, project.artworkAnalysis?.edgeMetrics);
  return <section className="flow-screen"><Progress screen="confirm" /><FlowHeader eyebrow="SON KONTROL" title="Mockupları oluşturmadan önce kontrol edin." copy="Oluştur düğmesi yalnızca bir kez altı bağımsız işi başlatır. Bu kurulum demo modundadır ve gerçek Fal.ai bakiyesi kullanmaz." /><div className="demo-banner"><b>Demo Mode — Real Fal.ai generation is not connected</b><span>Gösterilen çıktılar yerel önizlemedir; gerçek üretim sonucu olarak sunulmaz.</span></div><div className="confirmation-grid"><div className="confirm-art"><img src={master.previewUrl || master.fileUrl} alt="Onaylanacak kaynak duvar kâğıdı" /><span>SOURCE ARTWORK · v{master.version}</span></div><div className="confirm-ledger"><div><span>Product</span><b>{project.productType === "seamless" ? "Seamless Pattern" : "Custom Mural"}</b></div><div><span>Primary room</span><b>{project.primaryTargetRoom}</b></div><div><span>Secondary room</span><b>{project.secondaryTargetRoom || "None"}</b></div><div><span>{project.productType === "seamless" ? "Pattern scale" : "Placement"}</span><b>{project.productType === "seamless" ? project.patternScale : project.artworkPlacementMode.replaceAll("_", " ")}</b></div><div><span>Mockup set</span><b>6 independent scenes</b></div><div><span>Fal model</span><b>Server-selected after connection</b></div><div><span>Estimated cost</span><b>Available after secure Fal.ai connection</b></div><div><span>Artwork QA</span><b>{qa.status === "QA_PASSED" ? `Passed · ${qa.score}/100` : `Review recommended · ${qa.score}/100`}</b></div></div></div><div className="qa-mini"><b>QA checks</b>{qa.checks.map((item) => <span key={item.key} data-state={item.status}>{item.status === "pass" ? "✓" : item.status === "warn" ? "!" : "×"} {item.label}</span>)}</div><div className="form-actions"><button className="secondary-button" onClick={back}>Ayarları Düzenle</button><button className="primary-button" onClick={create}>6 Mockup Oluştur →</button></div></section>;
}

function Results({ project, update, render, busy, openPreview, openHelp }: { project: Project; update: (project: Project) => void; render: (ids: string[]) => Promise<void>; busy: boolean; openPreview: (asset: OutputAsset) => void; openHelp: (section: string) => void }) {
  ...
  const statusLabel = (status: Project["slots"][number]["status"]) => status === "ready" ? "Completed" : status === "failed" ? "Failed" : status === "queued" ? "Generating" : "Waiting";
  return <section className="flow-screen"><Progress screen="results" /><FlowHeader eyebrow="SONUÇLAR" title="Altı sahneyi ayrı ayrı yönetin." copy="Her sonuç aynı kaynak görsele bağlıdır. Uygun olanı onaylayın; yalnızca başarısız veya reddedilmiş bir sahneyi yeniden oluşturun." /><div className="demo-banner"><b>Demo Mode — Real Fal.ai generation is not connected</b><span>DEMO OUTPUT etiketli görseller gerçek Fal.ai üretimi değildir.</span><button onClick={() => openHelp("fal-ai")}>Bağlantı nasıl çalışır?</button></div><div className="results-toolbar"><span><b>{approved.length}</b> / 6 approved</span><button disabled={!selected.length || exporting} onClick={() => exportAssets(selected, "selected-wallpaper-mockups.zip")}>Selected ZIP</button><button className="primary-button" disabled={!approved.length || exporting} onClick={() => exportAssets(approved.map((asset) => asset.id), "approved-wallpaper-mockups.zip")}>ZIP all approved ↓</button></div><div className="result-grid">{project.slots.map((slot, index) => { const asset = project.outputAssets.find((item) => item.id === slot.outputAssetId); const canRetry = slot.status === "failed" || Boolean(asset?.rejected); return <article key={slot.id} className={asset?.rejected ? "rejected" : asset?.approved ? "approved" : ""}><button className="result-preview" disabled={!asset} onClick={() => asset && openPreview(asset)} style={asset ? { backgroundImage: `url(${asset.fileUrl})` } : undefined}><span>M{index + 1}</span><b>{statusLabel(slot.status)}</b>{asset && <em>DEMO OUTPUT</em>}</button><div className="result-copy"><small>{slot.role}</small><h3>{asset ? `${asset.width} × ${asset.height} JPG` : "Render sırasında güncellenecek"}</h3>{asset && <label><input type="checkbox" checked={selected.includes(asset.id)} onChange={() => toggle(asset.id)} /> ZIP için seç</label>}</div><div className="result-actions">{asset && <button onClick={() => update(approveOutput(project, asset.id, true))}>{asset.approved ? "Approved ✓" : "Approve"}</button>}{asset && <button onClick={() => update(approveOutput(project, asset.id, false))}>Reject</button>}<button disabled={!canRetry || busy} onClick={() => render([slot.id])}>Regenerate</button>{asset && <button onClick={async () => { const response = await fetch(asset.fileUrl); downloadBlob(await response.blob(), asset.fileName); }}>Download</button>}</div></article>; })}</div></section>;
}
...
function normalizeProject(raw: Project) {
  ...
  return { ...fallback, ...raw, productionMaster: null, activeMasterVersionId: null, masterVersions: [], outputAssets: [], renderJobs: [], slots: fallback.slots, masterStatus: "AWAITING_UPLOAD" as const };
}

export default function StudioApp() {
  const [project, setProject] = useState<Project | null>(null); const [screen, setScreen] = useState<Screen>("home"); const [uploadMode, setUploadMode] = useState<UploadMode>("direct"); const [helpOpen, setHelpOpen] = useState(false); const [helpSection, setHelpSection] = useState<string | null>(null); const [rendering, setRendering] = useState(false); const [preview, setPreview] = useState<OutputAsset | null>(null);
  ...
  const render = async (ids: string[]) => { if (!project?.productionMaster || rendering) return; setRendering(true); try { const qa = evaluateMasterQa(project, project.productionMaster, project.artworkAnalysis?.edgeMetrics); const approvedAt = new Date().toISOString(); const master = { ...project.productionMaster, approvedAt }; const prepared: Project = { ...project, qa, masterStatus: "APPROVED", productionMaster: master, masterVersions: project.masterVersions.map((item) => item.id === master.id ? master : item), artDirection: { ...project.artDirection, savedAt: project.artDirection.savedAt || approvedAt } }; const requested = requestRenderJobs(prepared, prepared.userId, ids, "mock"); let current = requested.project; update(current); setScreen("results"); for (const job of requested.jobs) { const output = await compositeOutput(current, job); current = completeRenderJob(current, job.jobId, output); update(current); } } finally { setRendering(false); } };
  const resumeScreen = useMemo<Screen>(() => project?.outputAssets.length ? "results" : project?.productionMaster ? "settings" : "home", [project]);
  if (!project) return <div className="loading"><span>W</span><p>Stüdyo hazırlanıyor…</p></div>;
  return <div className="simple-app"><header className="simple-topbar"><button className="simple-brand" onClick={() => setScreen("home")}><span>W</span><strong>Wallpaper AI Studio<small>MOCKUP WORKSPACE</small></strong></button><div className="top-project"><small>{project.productionMaster ? "ACTIVE PROJECT" : "NEW WORKSPACE"}</small><b>{project.productionMaster ? project.projectName : "Choose how to begin"}</b></div><button className="new-button" onClick={startFresh}>＋ Yeni çalışma</button></header><div className="simple-shell"><aside className="simple-sidebar"><nav aria-label="Ana menü"><button className={screen === "home" ? "active" : ""} onClick={() => setScreen("home")}><span>⌂</span>Ana Sayfa</button><button className={screen === "projects" ? "active" : ""} onClick={() => setScreen("projects")}><span>□</span>Projelerim</button><button onClick={() => openHelp()}><span>?</span>Kullanım Kılavuzu</button><button className={screen === "account" ? "active" : ""} onClick={() => setScreen("account")}><span>⚙</span>Ayarlar</button></nav><div className="mode-card"><b>DEMO MODE</b><span>Fal.ai bağlı değil</span></div></aside><main className="simple-main"><div className="simple-content">
    ...
    {screen === "confirm" && project.productionMaster && <Confirmation project={project} back={() => setScreen("settings")} create={() => render(project.slots.map((slot) => slot.id))} />}
    {screen === "results" && <Results project={project} update={update} render={render} busy={rendering} openPreview={setPreview} openHelp={openHelp} />}
