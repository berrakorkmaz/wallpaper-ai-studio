...
async function sourceDataUrl(fileUrl: string) {
  ...
  const blob = await response.blob(); if (!blob.size || blob.size > 20 * 1024 * 1024) throw new Error("Source wallpaper must be smaller than 20 MB for local Fal compositing.");
  const image = new Image(); await new Promise<void>((resolve, reject) => { image.onload = () => resolve(); image.onerror = reject; image.src = fileUrl; });
  if (!image.naturalWidth || !image.naturalHeight || image.naturalWidth * image.naturalHeight > 25_000_000) throw new Error("Source wallpaper pixel dimensions are too large for local compositing.");
  ...
  const png = await new Promise<Blob>((resolve, reject) => canvas.toBlob((value) => value ? resolve(value) : reject(new Error("Source wallpaper could not be encoded.")), "image/png"));
  return await new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onload = () => typeof reader.result === "string" ? resolve(reader.result) : reject(new Error("Source wallpaper could not be encoded.")); reader.onerror = () => reject(new Error("Source wallpaper could not be encoded.")); reader.readAsDataURL(png); });
}
...
function Confirmation({ project, back, create, singleMockupTest }: { project: Project; back: () => void; create: () => void; singleMockupTest: boolean }) {
  const master = project.productionMaster!;
  const qa = evaluateMasterQa(project, master, project.artworkAnalysis?.edgeMetrics);
  return <section className="flow-screen"><Progress screen="confirm" /><FlowHeader eyebrow="SON KONTROL" title="Mockupları oluşturmadan önce kontrol edin." copy={singleMockupTest ? "Wall masking ve foreground occlusion doğrulaması için yalnızca ilk scene blueprint'i üretilecek." : "Oluştur düğmesi yalnızca bir kez altı bağımsız işi başlatır. Bu kurulum demo modundadır ve gerçek Fal.ai bakiyesi kullanmaz."} /><div className="demo-banner"><b>{singleMockupTest ? "Single Mockup · Semantic Wall Mask Test" : "Demo Mode — Real Fal.ai generation is not connected"}</b><span>{singleMockupTest ? "Fal temiz odayı ve semantik duvar maskesini üretir; orijinal wallpaper yalnızca maskenin içine uygulanır." : "Gösterilen çıktılar yerel önizlemedir; gerçek üretim sonucu olarak sunulmaz."}</span></div><div className="confirmation-grid"><div className="confirm-art"><img src={master.previewUrl || master.fileUrl} alt="Onaylanacak kaynak duvar kâğıdı" /><span>SOURCE ARTWORK · v{master.version}</span></div><div className="confirm-ledger"><div><span>Product</span><b>{project.productType === "seamless" ? "Seamless Pattern" : "Custom Mural"}</b></div><div><span>Primary room</span><b>{project.primaryTargetRoom}</b></div><div><span>Secondary room</span><b>{project.secondaryTargetRoom || "None"}</b></div><div><span>{project.productType === "seamless" ? "Pattern scale" : "Placement"}</span><b>{project.productType === "seamless" ? project.patternScale : project.artworkPlacementMode.replaceAll("_", " ")}</b></div><div><span>Mockup set</span><b>{singleMockupTest ? "1 semantic-mask test scene" : "6 independent scenes"}</b></div><div><span>Fal model</span><b>{singleMockupTest ? "Scene + wall segmentation" : "Not connected"}</b></div><div><span>Artwork QA</span><b>{qa.status === "QA_PASSED" ? `Passed · ${qa.score}/100` : `Review recommended · ${qa.score}/100`}</b></div></div></div><div className="qa-mini"><b>QA checks</b>{qa.checks.map((item) => <span key={item.key} data-state={item.status}>{item.status === "pass" ? "✓" : item.status === "warn" ? "!" : "×"} {item.label}</span>)}</div><div className="form-actions"><button className="secondary-button" onClick={back}>Ayarları Düzenle</button><button className="primary-button" onClick={create}>{singleMockupTest ? "1 Masked Mockup Test Oluştur →" : "6 Mockup Oluştur →"}</button></div></section>;
}

function Results({ project, update, render, retry, stages, generationError, mode, provider, singleMockupTest, busy, openPreview, openHelp }: { project: Project; update: (project: Project) => void; render: (ids: string[], sourceProject?: Project) => Promise<void>; retry: (slotId: string) => Promise<void>; stages: Record<string, MockupJobStage>; generationError: string; mode: "demo" | "production"; provider: "mock" | "fal" | "custom"; singleMockupTest: boolean; busy: boolean; openPreview: (asset: OutputAsset) => void; openHelp: (section: string) => void }) {
...
export default function StudioApp() {
  ...
  return <div className="simple-app"><header className="simple-topbar"><button className="simple-brand" onClick={() => setScreen("home")}><span>W</span><strong>Wallpaper AI Studio<small>MOCKUP WORKSPACE</small></strong></button><div className="top-project"><small>{project.productionMaster ? "ACTIVE PROJECT" : "NEW WORKSPACE"}</small><b>{project.productionMaster ? project.projectName : "Choose how to begin"}</b></div><button className="new-button" onClick={startFresh}>＋ Yeni çalışma</button></header><div className="simple-shell"><aside className="simple-sidebar"><nav aria-label="Ana menü"><button className={screen === "home" ? "active" : ""} onClick={() => setScreen("home")}><span>⌂</span>Ana Sayfa</button><button className={screen === "projects" ? "active" : ""} onClick={() => setScreen("projects")}><span>□</span>Projelerim</button><button onClick={() => openHelp()}><span>?</span>Kullanım Kılavuzu</button><button className={screen === "account" ? "active" : ""} onClick={() => setScreen("account")}><span>⚙</span>Ayarlar</button></nav><div className="mode-card"><b>Render Provider: {renderConfig.provider === "fal" ? "Fal" : renderConfig.provider === "custom" ? "Custom" : "Mock"}</b><span>{renderConfig.provider === "fal" ? renderConfig.configured ? "Real AI renderer" : "Missing FAL_KEY" : renderConfig.provider === "custom" ? "Production renderer" : "Development renderer"}</span></div></aside><main className="simple-main"><div className="simple-content">
    ...
    {screen === "confirm" && project.productionMaster && <Confirmation project={project} back={() => setScreen("settings")} create={() => render(project.slots.map((slot) => slot.id))} singleMockupTest={renderConfig.falSingleMockupTest} />}
    {screen === "results" && <Results project={project} update={update} render={render} retry={retry} stages={Object.fromEntries((remoteBatch?.outputs ?? []).map((output) => [output.slotId, output.status]))} generationError={generationError} mode={renderConfig.mode} provider={renderConfig.provider} singleMockupTest={renderConfig.falSingleMockupTest} busy={rendering} openPreview={setPreview} openHelp={openHelp} />}
