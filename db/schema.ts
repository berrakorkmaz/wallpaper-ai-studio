...
export const designAssets = sqliteTable("design_assets", {
  id: text("id").primaryKey(), projectId: text("project_id").notNull().references(() => projects.id), userId: text("user_id").notNull().references(() => users.id), role: text("role").notNull(), fileUrl: text("file_url").notNull(), fileName: text("file_name").notNull(), mimeType: text("mime_type").notNull(), fileSize: integer("file_size").notNull(), width: integer("width").notNull(), height: integer("height").notNull(), fileHash: text("file_hash").notNull(), version: integer("version").notNull(), masterStatus: text("master_status"), storageKey: text("storage_key"), signedSourceUrl: text("signed_source_url"), immutable: integer("immutable", { mode: "boolean" }).notNull().default(true), approvedAt: integer("approved_at", { mode: "timestamp_ms" }), createdAt: integer("created_at", { mode: "timestamp_ms" }).notNull(),
...
export const mockupBatches = sqliteTable("mockup_batches", {
  id: text("id").primaryKey(), projectId: text("project_id").notNull().references(() => projects.id), userId: text("user_id").notNull().references(() => users.id), masterVersionId: text("master_version_id").notNull().references(() => designAssets.id), provider: text("provider").notNull(), status: text("status").notNull(), idempotencyKey: text("idempotency_key").notNull(), createdAt: integer("created_at", { mode: "timestamp_ms" }).notNull(), updatedAt: integer("updated_at", { mode: "timestamp_ms" }).notNull(),
}, (table) => [index("idx_mockup_batches_owner_project").on(table.userId, table.projectId), uniqueIndex("mockup_batches_idempotency_unique").on(table.idempotencyKey)]);

export const mockupOutputs = sqliteTable("mockup_outputs", {
  id: text("id").primaryKey(), jobId: text("job_id").notNull().references(() => mockupBatches.id), sceneId: text("scene_id").notNull(), slotId: text("slot_id").notNull(), category: text("category").notNull(), status: text("status").notNull(), prompt: text("prompt").notNull(), blueprint: text("blueprint", { mode: "json" }).notNull(), provider: text("provider").notNull(), providerJobId: text("provider_job_id"), outputStorageKey: text("output_storage_key"), thumbnailStorageKey: text("thumbnail_storage_key"), width: integer("width").notNull(), height: integer("height").notNull(), error: text("error", { mode: "json" }), createdAt: integer("created_at", { mode: "timestamp_ms" }).notNull(), updatedAt: integer("updated_at", { mode: "timestamp_ms" }).notNull(),
}, (table) => [index("idx_mockup_outputs_job_slot").on(table.jobId, table.slotId), index("idx_mockup_outputs_provider_job").on(table.providerJobId)]);
