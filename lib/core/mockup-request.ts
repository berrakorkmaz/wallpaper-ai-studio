...
export function buildMockupBatchRequest(project: Project, slotIds = project.slots.map((slot) => slot.id)): CreateMockupBatchRequest {
  ...
  return {
    ...
    source: { assetId: master.id, storageKey: master.storageKey ?? "", signedSourceUrl: master.signedSourceUrl ?? "", fileHash: master.fileHash, mimeType: master.mimeType, width: master.width, height: master.height },
