...
export function buildMockupPrompt({ project, sceneBlueprint, previousScenes = [] }: { project: Project; sceneBlueprint: SceneBlueprint; previousScenes?: SceneBlueprint[] }) {
  ...
  const themeDirection = project.prompt.theme === "Disney" ? ` Artwork theme context: Disney, character/story selection ${project.prompt.customCharacterStory || project.prompt.characterStory || "storybook"}, pattern content ${project.prompt.patternContent}.` : ` Artwork theme context: ${project.prompt.theme}.`;
  ...
  return `Create a photorealistic ${sceneBlueprint.roomType} wallpaper mockup. Preserve the supplied wallpaper artwork exactly: keep motif identity, colors, pattern geometry and believable scale; apply it only to the wall plane with correct perspective.${themeDirection} Category direction: ${category.visualDirection}. Scene role: ${sceneBlueprint.roleLabel} — ${sceneBlueprint.title}. Camera: ${sceneBlueprint.cameraAngle}, ${sceneBlueprint.cameraDistance}, ${sceneBlueprint.cameraHeight}, ${sceneBlueprint.lensStyle}. Composition: ${sceneBlueprint.composition}. Architecture: ${sceneBlueprint.architecture}. Furniture: ${sceneBlueprint.furnitureStyle}; include ${sceneBlueprint.keyFurniture.join(" and ")}. Props: ${sceneBlueprint.props.join(", ")}. Lighting: ${sceneBlueprint.lighting}. Mood: ${sceneBlueprint.mood}. Wallpaper coverage: ${sceneBlueprint.wallpaperCoverage}. Avoid: ${sceneBlueprint.negativeConstraints.join("; ")}. Do not reproduce the previous room composition.${prior}`;
}
