...
export function getServerRenderConfig(env: NodeJS.ProcessEnv = process.env): ServerRenderConfig {
  ...
  return { provider, demoMode: provider === "fal" ? false : demoMode, serviceUrl, serviceToken, falModel, falConfigured: provider === "fal" && !missing.length, productionReady: provider === "fal" ? !missing.length : !demoMode && provider !== "mock" && !missing.length, missing };
}
